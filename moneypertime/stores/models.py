...
class Store(models.Model):
    ...
    price = models.FloatField(help_text='Price of the store itself')
    ...
    build_time = models.FloatField(help_text='Time spent to build this store')
    should_create_on_water = models.BooleanField(help_text='Should the store be created on water?', default=False)
    amount = models.FloatField(help_text='Amount earned per interval')
    ...
    def performance(self):
        return self.amount / (self.interval * self.size1 * self.size2)
